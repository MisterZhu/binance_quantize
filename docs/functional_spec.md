...
页面内涉及 Binance 接口、行情加载、API 检测、持仓读取、配置保存、策略切换和机器人控制的操作，都应显示明确 loading 文案。关键接口操作使用展开状态块显示“进行中 / 完成 / 失败”，并在页面顶部保留最近一次操作结果。
...
- 支撑/压力
- 候选盈亏比
- 信号生成时使用的策略名称、大类、方向模式和周期

注意：信号页展示的是数据库里某条历史信号生成时保存的检查项，不会因为你在策略页切换策略而自动改写。切换策略后，需要等待机器人下一轮循环生成新信号，或点击“按当前策略刷新信号”手动生成当前策略的检查项预览。

“按当前策略刷新信号”只重新拉取行情、运行当前策略并写入 `signals` 表，不执行风控、不下单。

信号页默认只展示当前启用策略生成的信号，因此趋势回调类只显示该类策略的 5 项做多和 5 项做空检查项。需要排查旧信号时，可以切换到“全部历史”。
...
策略页使用树状二级分类：一级节点是策略大类，例如“趋势突破类”；展开后显示该大类下的二级策略，例如“AI推荐-合约结构突破回踩”。点击二级策略后展示详情和参数，当前选中的二级策略按钮使用高亮背景。不同大类展示不同的入场条件开关。
